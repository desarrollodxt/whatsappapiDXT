...
class Indicadores extends CI_Controller
{
    ...
    public function detalleMetrica($id_usuario, $metrica)
    {
        ...
        switch ($metrica) {
            ...
            case 'clientesActivos':
                $data = $this->Indicadores_model->getClientesActivosPUsuarioDetalle($usuario_rainde, $roles);
                break;
            case 'metaGlobal':
                $data = $this->Indicadores_model->getobjetivoGlobalProfitDetalle($usuario_rainde, $roles);
                break;
            case 'cartera':
                $data = $this->Indicadores_model->getCarteraPUsuarioDetalle($usuario_rainde, $roles);
                break;
            default:
    ...
    public function getIndicadoresAgentecuentas($id)
    {
        $this->load->model('Indicadores_model');
        $this->load->model('Usuario_model');
        $getUsuario = $this->Usuario_model->getUsuario($id);
        ...
        $cumplimientoMeta = $this->Indicadores_model->getCumplimientoMetaPUsuario($usuario_rainde, $roles);
        $vtaPerdida = $this->Indicadores_model->getVtaPerdidaPUsuario($usuario_rainde, $roles);
        $proveedoresActivosPausadosPerdidos = $this->Indicadores_model->getClientesActivos($usuario_rainde, $roles);
        $comisiones = $this->Indicadores_model->getComisionesPUsuario($usuario_rainde, $roles, $cumplimientoMeta, $objetivoGlobal);
        $cartera = $this->Indicadores_model->getCarteraPUsuario($usuario_rainde, $roles);
        $data = [
            "profitMes" => $profitMes,
            "objetivoGlobal" => $objetivoGlobal,
            "recuperacion" => $recuperacion,
